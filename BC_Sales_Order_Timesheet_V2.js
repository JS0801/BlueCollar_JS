...
define([
  ...
  function buildConsolidatedInvoiceData(tranIds) {
    ...
    invoiceData.invoiceBlockHtml = buildInvoiceBlockHtml(invoiceData);
    return invoiceData;
  }

  function buildInvoiceBlockHtml(invoiceData) {
    var rowsHtml = '';
    for (var i = 0; i < invoiceData.categoryLines.length; i++) {
      var line = invoiceData.categoryLines[i];
      rowsHtml += ''
        + '<tr>'
        + '<td colspan="8" style="border-right:0px; border-top:0px; border-bottom:1px solid #C9C9C9;">' + line.description + '</td>'
        + '<td colspan="2" style="border-right:0px; border-top:0px; border-left:0px; border-bottom:1px solid #C9C9C9;" align="right">' + money(line.amount) + '</td>'
        + '<td colspan="2" style="border-right:0px; border-top:0px; border-left:0px; border-bottom:1px solid #C9C9C9;" align="center">' + pct(line.taxRate) + '</td>'
        + '<td colspan="2" style="border-right:0px; border-top:0px; border-left:0px; border-bottom:1px solid #C9C9C9;" align="right">' + money(line.taxAmount) + '</td>'
        + '<td colspan="2" style="border-left:0px; border-top:0px; border-bottom:1px solid #C9C9C9;" align="right">' + money(line.lineTotal) + '</td>'
        + '</tr>';
    }
    ...
    var invoiceBlockHtml = ''
      ...
      + '<tr><td colspan="6" valign="top" style="border:none;"><b>PO Number:</b><br/>' + esc(invoiceData.poNumbers.join(', ')) + '</td></tr>'
      + '<tr><td colspan="6" valign="top" style="border:none;"><b>Customer Reference:</b><br/>' + esc(invoiceData.projectRefs.join(', ')) + '</td></tr>'

      + '<tr><td colspan="16" style="border:none;">&nbsp;</td></tr>'
      + '<tr><td colspan="16" style="border:none;"><b>Memo:</b><br/>' + esc(invoiceData.memos.join(' | ')) + '</td></tr>'
      + '<tr><td colspan="16" style="border:none;">&nbsp;</td></tr>'
      ...
      + '<th colspan="2" align="center" style="border-top:0px; border-right:0px; border-left:0px; border-bottom:1px solid #999;"><b>' + (invoiceData.isAmericas ? 'TAX' : 'GST') + '</b></th>'
      + '<th colspan="2" align="right" style="border-top:0px; border-right:0px; border-left:0px; border-bottom:1px solid #999;"><b>' + invoiceData.TAX_LABEL_AMT + '</b></th>'
      + '<th colspan="2" align="right" style="border-top:0px; border-left:0px; border-bottom:1px solid #999;"><b>Amount ' + invoiceData.currencyText + '</b></th>'
      + '</tr>'

      + rowsHtml

      + '<tr><td colspan="10" style="border:none;">&nbsp;</td></tr>'
      + '<tr><td rowspan="3" colspan="12" style="border:none;"></td><td colspan="2" align="right" style="border:none;">Subtotal</td><td colspan="2" align="right" style="border:none;">' + money(invoiceData.subTotalExTax) + '</td></tr>'
      + '<tr><td colspan="2" align="right" style="border:none;">' + invoiceData.TAX_LABEL_TOTAL + '</td><td colspan="2" align="right" style="border:none;">' + money(Math.abs(invoiceData.totalTax)) + '</td></tr>'
      + '<tr><td colspan="2" align="right" style="border-top:1px solid #999; border-left:0px; border-right:0px; border-bottom:0px;"><b>TOTAL ' + invoiceData.currencyText + '</b></td><td colspan="2" align="right" style="border-top:1px solid #999; border-right:0px; border-left:0px; border-bottom:0px;"><b>' + money(invoiceData.grandTotal) + '</b></td></tr>'
  ...
  function buildTimesheetData(soId, empNameArr) {
    ...
    log.debug('groupedFinalArray After Merge', groupedFinalArray);
  ...
  function buildInvoiceExcelBlock(invoiceData, tracker) {
    var html = '';
    ...
    html += '<td colspan="5" rowspan="4" valign="top" style="border:none;">'
      + '<b>ATTN:</b><br/>' + invoiceData.billAddrHtml + '</td>';
    ...
    html += '<td colspan="5" rowspan="4" align="right" valign="top" style="border:none;">'
      + invoiceData.subAddrHtml + '<br/><b>ABN:</b> ' + invoiceData.subABN + '</td>';
    html += '</tr>';
    ...
    var invoiceLineStartRow = tracker.row;
    for (var i = 0; i < invoiceData.categoryLines.length; i++) {
      var lineRow = getCurrentRowNumber(tracker);
      var line = invoiceData.categoryLines[i];
      ...
      html += '<td colspan="2" style="border-left:0px; border-top:0px; border-bottom:1px solid #C9C9C9; ' + excelCurrencyStyle() + '" align="right">='
        + getCellRef(lineRow, 9) + '+' + getCellRef(lineRow, 13) + '</td>';
      html += '</tr>';
    }
    var invoiceLineEndRow = tracker.row - 1;

    html += makeRowStart(tracker);
    html += '<td colspan="10" style="border:none;">&nbsp;</td>';
    html += '</tr>';

    var subtotalRow = getCurrentRowNumber(tracker);
    html += makeRowStart(tracker);
    ...
    var taxRow = getCurrentRowNumber(tracker);
    html += makeRowStart(tracker);
  ...
  function buildTimesheetHtmlBlock(ts, tracker) {
    ...
    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Client:</td><td style="border:1px solid #000;" colspan="4">' + esc(h.client) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Customer Ref #:</td><td style="border:1px solid #000;" colspan="4">' + esc(h.customerRef) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Week-Ending:</td><td style="border:1px solid #000; mso-number-format:\\@;" colspan="4">="' + formatDateMMDDYYYY(h.weekEnding) + '"</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">C2O Project Manager:</td><td style="border:1px solid #000;" colspan="4">' + esc(h.projectManager) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Description of Work:</td><td style="border:1px solid #000;" colspan="4">' + esc(h.description) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Document #:</td><td style="border:1px solid #000;" colspan="4">' + esc(h.docNumber) + '</td>';
    html += '</tr>';
    ...
    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Project:</td><td colspan="4" style="border:1px solid #000;">' + esc(h.reportingProject) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">C2O Job:</td><td colspan="4" style="border:1px solid #000;">' + esc(h.projectName) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Supervisor:</td><td colspan="4" style="border:1px solid #000;">' + esc(h.supervisor) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Start Time: Monday – Friday</td><td style="border:1px solid #000;">' + esc(h.startTime) + '</td><td class="info-header" colspan="2">Finish Time:</td><td style="border:1px solid #000;">' + esc(h.endTime) + '</td>';
    html += '</tr>';

    html += makeRowStart(tracker);
    html += '<td class="info-header" colspan="2">Start Time: Weekend / Holiday</td><td style="border:1px solid #000;">' + esc(h.startTime) + '</td><td class="info-header" colspan="2">Finish Time:</td><td style="border:1px solid #000;">' + esc(h.endTime) + '</td>';
    html += '</tr>';
    ...
    if (x.Labor || x.Labour) {
      ...
      var laborDataStartRow = tracker.row;

      for (var q = 1; q < labor.length - 1; q++) {
        var rowNum = getCurrentRowNumber(tracker);
        ...
        for (var w = 0; w < labor[q].days.length; w++) {
          html += '<td style="' + excelNumberStyleOneDecimal() + '">' + labor[q].days[w].hours + '</td>';
        }

        html += '<td style="' + excelNumberStyleOneDecimal() + '">=SUM('
      ...
      if (labor.length > 1) {
        var laborDataEndRow = tracker.row - 1;
        var totalRowNum = getCurrentRowNumber(tracker);
        var last = labor[labor.length - 1];
        var lastTotalWeekCol = laborFirstDayCol + last.days.length;
        var lastRateCol = lastTotalWeekCol + 1;
        var lastAmtCol = lastRateCol + 1;

        html += makeRowStart(tracker);
        html += '<td colspan="5" style="border-left:0; border-bottom:0;"></td>';
        html += '<td class="table-header"><b>' + last.employee + '</b></td>';

        for (var lw = 0; lw < last.days.length; lw++) {
          var dayCol = laborFirstDayCol + lw;
          html += '<td class="table-header" style="' + excelNumberStyleOneDecimal() + '"><b>=SUM('
            + getCellRef(laborDataStartRow, dayCol) + ':' + getCellRef(laborDataEndRow, dayCol) + ')</b></td>';
        }

        html += '<td class="table-header" style="' + excelNumberStyleOneDecimal() + '"><b>=SUM('
          + getCellRef(laborDataStartRow, lastTotalWeekCol) + ':' + getCellRef(laborDataEndRow, lastTotalWeekCol) + ')</b></td>';

        html += '<td class="table-header"><b></b></td>';

        html += '<td class="table-header" style="' + excelCurrencyStyle() + '"><b>=SUM('
          + getCellRef(laborDataStartRow, lastAmtCol) + ':' + getCellRef(laborDataEndRow, lastAmtCol) + ')</b></td>';
    ...
    if (x['Equipment / Vehicle Rental']) {
      ...
      var eqFirstDayCol = 5;
      var eqDataStartRow = tracker.row;

      for (var r = 1; r < equp.length - 1; r++) {
        var eqRowNum = getCurrentRowNumber(tracker);
        ...
        for (var t = 0; t < equp[r].days.length; t++) {
          html += '<td style="' + excelNumberStyleOneDecimal() + '">' + equp[r].days[t].hours + '</td>';
        }
      ...
      if (equp.length > 1) {
        var eqDataEndRow = tracker.row - 1;
        var eqLast = equp[equp.length - 1];
        var eqLastTotalWeekCol = eqFirstDayCol + eqLast.days.length;

        html += makeRowStart(tracker);
        html += '<td colspan="4" class="table-header"><b>' + eqLast.role + '</b></td>';

        for (var te = 0; te < eqLast.days.length; te++) {
          var eqDayCol = eqFirstDayCol + te;
          html += '<td class="table-header" style="' + excelNumberStyleOneDecimal() + '"><b>=SUM('
            + getCellRef(eqDataStartRow, eqDayCol) + ':' + getCellRef(eqDataEndRow, eqDayCol) + ')</b></td>';
        }

        html += '<td class="table-header" style="' + excelNumberStyleOneDecimal() + '"><b>=SUM('
          + getCellRef(eqDataStartRow, eqLastTotalWeekCol) + ':' + getCellRef(eqDataEndRow, eqLastTotalWeekCol) + ')</b></td>';
    ...
    if (x.Materials) {
      ...
      var matDataStartRow = tracker.row;
      ...
      if (tracker.row > matDataStartRow) {
        var matDataEndRow = tracker.row - 1;
    ...
    if (x.Expenses) {
      ...
      var expDataStartRow = tracker.row;
      ...
      if (tracker.row > expDataStartRow) {
        var expDataEndRow = tracker.row - 1;
  ...
  function excelNumberStyleOneDecimal() {
    return 'mso-number-format:"0.0";';
  }

  function getCurrentRowNumber(tracker) {
    return tracker.row;
  }

  function makeRowStart(tracker) {
    var r = tracker.row;
    tracker.row = r + 1;
    return '<tr>';
  }
